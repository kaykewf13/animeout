...
function buildSources(episode){
  ...
  (episode?.alternatives || []).forEach(a => {
    if(a?.url && isPlayableUrl(a.url) && !sources.some(s => s.url === a.url)) {
      sources.push({ url: a.url, source: a.source || 'fallback', score: a.score || 0, subtitle: a.subtitle || '' });
    }
  });
...
function buildCategories(){
  ...
  [...new Set(state.items.map(i=>i.category).filter(Boolean))].sort().forEach(c=>{
...
function setHero(item){
  ...
  $('heroSubtitle').textContent = item.description || (eps ? `${eps} episódio(s) disponíveis.` : 'Catálogo atualizado automaticamente com listas M3U e player premium.');
  const hero = $('hero');
...
function resetVideo(){ const video = $('videoPlayer'); clearTracks(video); if(window.hls){ window.hls.destroy(); window.hls = null; } video.onerror = null; video.onstalled = null; video.onended = null; video.removeAttribute('src'); video.load(); return video; }

function play(item, episodeIndex = 0){
  state.currentItem = item; state.currentEpisodeIndex = episodeIndex; state.currentSourceIndex = 0; state.retryCount = 0;
  setHero(item); renderEpisodeList(item); renderRecommendations(item);
  const episodes = getEpisodes(item);
  const selected = episodes.length ? episodes[episodeIndex] : getPlayable(item);
  state.currentSources = buildSources(selected);
...
function playSource(episode, source){
  ...
  $('nowTitle').textContent = episode.title || state.currentItem?.title || 'Reproduzindo';
  $('nowMeta').textContent = state.currentItem?.groupTitle || state.currentItem?.category || '';
  $('nowDescription').textContent = state.currentItem?.description || (getEpisodes(state.currentItem).length ? `${getEpisodes(state.currentItem).length} episódio(s) disponíveis.` : '');
  $('nowSource').textContent = `Fonte: ${source.source || 'principal'} • score ${source.score || 0}`;
  addSubtitle(video, source.subtitle || episode.subtitle);
  if(!url){ return failover('URL indisponível'); }
  if(isPlaylistOnly(url)){ return failover('Playlist .m3u não roda no player web'); }
  if(!isPlayableUrl(url)){ return failover('Formato não suportado'); }
  const clean = url.split('?')[0].toLowerCase();
  video.onerror = () => failover('Erro no vídeo');
  video.onstalled = () => setTimeout(()=>{ if(video.readyState < 2) failover('Stream travado'); }, 8000);
...
function failover(reason){
  const episode = getEpisodes(state.currentItem)[state.currentEpisodeIndex] || getPlayable(state.currentItem);
  const source = state.currentSources[state.currentSourceIndex];
  if(state.retryCount < state.maxRetries){ state.retryCount++; $('nowSource').textContent = `${reason}. Tentando novamente (${state.retryCount}/${state.maxRetries})...`; setTimeout(()=>playSource(episode, source), 1200); return; }
  state.currentSourceIndex++;
  if(state.currentSourceIndex < state.currentSources.length){ state.retryCount = 0; $('nowSource').textContent = `${reason}. Trocando para backup ${state.currentSourceIndex+1}/${state.currentSources.length}...`; setTimeout(()=>playSource(episode, state.currentSources[state.currentSourceIndex]), 900); return; }
...
function renderEpisodeList(item){
  const panel = $('episodePanel'), list = $('episodeList'); if(!panel || !list) return; const eps = getEpisodes(item);
  if(!eps.length){ panel.classList.add('hidden'); list.innerHTML = ''; return; }
  panel.classList.remove('hidden'); $('episodeBadge').textContent = `${eps.length} eps`; list.innerHTML = '';
  eps.forEach((ep,idx)=>{ const btn = document.createElement('button'); btn.className = 'episodeButton'; btn.textContent = `Ep ${ep.episode || idx+1} • ${ep.title || item.title}`; btn.onclick = ()=>play(item, idx); list.appendChild(btn); });
}

function renderSourceSelector(){
  const container = $('sourceSelector'); if(!container) return;
  container.innerHTML = '';
  state.currentSources.forEach((s,i)=>{ const btn = document.createElement('button'); btn.className = i === state.currentSourceIndex ? 'sourceButton active' : 'sourceButton'; const quality = s.score > 90 ? '🟢 Alta' : s.score > 70 ? '🟡 Média' : '🔴 Baixa'; btn.textContent = `Fonte ${i+1} • ${quality}`; btn.onclick = ()=>{ state.currentSourceIndex = i; state.retryCount = 0; const ep = getEpisodes(state.currentItem)[state.currentEpisodeIndex] || getPlayable(state.currentItem); playSource(ep, s); renderSourceSelector(); }; container.appendChild(btn); });
}
